...
exports.postAddExpense = async(req,res)=>{
    ...
    try{
        ...
        const userTotal = await Users.findById(req.user._id);
        ...
        await Users.findByIdAndUpdate(req.user._id,{totalExpense:total},)
          await expense.save();
...
exports.deleteExpense = async(req,res)=>{
    ...
    session.startTransaction(); 
    try{
        const id = req.params._id; //when the user clicks on del button, a del req is sent with the id as a param,we extract it here
    ...
    await User.findByIdAndUpdate({id:req.user._id},{totalExpense:total},{session})
    await expense.findByIdAndDelete({id:req.params._id});//once its retrieved, its removed from the db using destroy
    await session.commitTransaction();
